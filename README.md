...
基于官方 Python SDK 的真实 `Choice` / `TypeSafeClient.system_one` 接口。仓库内脚本默认 **不访问** 付费服务：

```bash
uv run python examples/resource_triage.py          # mock 流程
uv run python examples/resource_triage.py --live   # 仅当已设置 TYPESAFE_API_KEY
```

示意见 [examples/resource_triage.py](examples/resource_triage.py)。Mock 只演示程序结构，**不是**真实 Jev 输出。
