...
| T08 | ETag 304 复用 | `sources/official.py` + `http_cache.py` | `tests/test_t08_etag.py::test_t08_*` | OfficialMonitorAdapter | PASS | 200→缓存；304+If-None-Match；缓存丢失重取 |
...
| T21 | 合并/关闭/分支删除恢复 | `Publisher._prepare_robot_catalog` | `test_publisher_rounds::test_t21_*`；`test_t21_merge_recovery.py::test_t21_*` | **生产 Publisher** | PASS | 合并/squash 续跑；PR head 恢复；recovery_gap；拒绝不被恢复 |
...
| T26 | 周报边界 | `rendering/weekly.py` | `tests/test_t26_weekly.py::test_t26_*` | write_weekly | PASS | 时区周界；半开；幂等；无空报；pending 隔离 |
...
- **PASS**：T01–T30（R1.1 已关闭原 T08/T21/T26 PARTIAL）
- 详见 `docs/implementation/r1/closeout.md`
